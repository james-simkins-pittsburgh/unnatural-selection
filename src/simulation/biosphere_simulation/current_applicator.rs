...
pub fn apply_current(
    ...
    for index in 0..all_spatial_biosphere_information.blob_vec[
        blob_number
    ].blob_members.len() as usize {
        let organism_num =
            all_spatial_biosphere_information.blob_vec[blob_number].blob_members[index];
        let organism_x = i64::from(
            all_spatial_biosphere_information.organism_information_vec[organism_num].x_location
        );
        let organism_y = i64::from(
            all_spatial_biosphere_information.organism_information_vec[organism_num].y_location
        );
        ...
        for current_number in 0..all_current_information.current_information_vec.len() as usize {
                    ...
                    (current_y - organism_y) * (current_y - organism_y) <= current_r * current_r
            {
                        ...
                        [organism_num].part_of_multi_org_blob
                {
                    ...
                    // Calculates angle to blob center
                    let angle_to_blob_center = if blob_x - organism_x > 0 {
                        deterministic_trig.d_trig.arctangent((
                            (((blob_y - organism_y) * 1000) / (blob_x - organism_x)) as i32,
                            1000,
                        )).0
                    } else if blob_x - organism_x < 0 {
                        deterministic_trig.d_trig.arctangent((
                            (((blob_y - organism_y) * 1000) / (blob_x - organism_x)) as i32,
                            1000,
                        )).0 + 3142
                    } else {
                        if blob_y - organism_y > 0 { 1571 } else { -1571 }
                    };
                    ...
                    all_spatial_biosphere_information.blob_vec[blob_number].blob_x_velocity +=
                        ((i64::from(
                            deterministic_trig.d_trig.cosine((angle_to_blob_center, 1000)).0
                        ) *
                    ...
                    all_spatial_biosphere_information.blob_vec[blob_number].blob_y_velocity +=
                        ((i64::from(
                            deterministic_trig.d_trig.sine((angle_to_blob_center, 1000)).0
                        ) *
                            translational_velocity_force) /
                            blob_mass) as i32;
                    1000;
                    ...
                    all_spatial_biosphere_information.blob_vec[blob_number].blob_x_velocity +=
                        ((i64::from(deterministic_trig.d_trig.cosine((current_a, 1000)).0) *
                            current_i) /
                            1000) as i32;
                    all_spatial_biosphere_information.blob_vec[blob_number].blob_y_velocity +=
                        ((i64::from(deterministic_trig.d_trig.sine((current_a, 1000)).0) *
                            current_i) /
                            1000) as i32;
                }
            }
